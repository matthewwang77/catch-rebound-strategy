...
**Goal:** 用 v7 优化参数，逐日模拟 2026-05-01 ~ 2026-06-15 的选股→AI分析→收益验证全流程，填入复盘界面。

**Architecture:** 独立脚本 `simulate_v7_from_may.py` 复用 `选股new_v5.py` 的 `_screen_single_stock()` + `auto_daily.py` 的 `_run_ai_analysis()` 逻辑。每交易日截断数据→筛选→AI分析→写入存储。不改动现有代码。

**Tech Stack:** Python, pandas, yfinance, DeepSeek API, importlib (动态导入选股new_v5和auto_daily)

**Skills to use:**
- `superpowers:test-driven-development` — 截断函数、历史市场函数
- `superpowers:subagent-driven-development` — 并行开发各组件
- `superpowers:systematic-debugging` — 遇到问题时根因分析
- memory 4-agent 并行审查工作流 — 代码完成后全量审查
...
```
simulate_v7_from_may.py  (新文件, ~400行)
  ├─ 导入: importlib 动态加载 选股new_v5, auto_daily
  ├─ truncate_df_to_date()           — 数据截断
  ├─ get_market_context_historical()  — 历史大盘上下文
  ├─ detect_regime_historical()       — 历史市场环境
  ├─ simulate_day()                   — 单日模拟
  └─ main()                           — 主循环
```
...
**Files:**
- Modify: `signal_tracker.csv`, `ai_memory.json`, `latest_scan_results.json`

- [ ] **Step 1: 备份并清空数据文件**

```bash
# 备份
cp signal_tracker.csv signal_tracker_v6_backup.csv
cp ai_memory.json ai_memory_v6_backup.json
cp latest_scan_results.json latest_scan_results_v6_backup.json 2>/dev/null || true

# 清空 signal_tracker.csv（保留表头）
python3 -c "
header = 'date,signal_date,code,mode,entry_price,pullback_pct,limit_days,name,sector\n'
with open('signal_tracker.csv', 'w') as f:
    f.write(header)
print('signal_tracker.csv 已清空（仅保留表头）')
"

# 清空 ai_memory.json
echo '{}' > ai_memory.json
echo 'ai_memory.json 已清空'

# 清空 latest_scan_results.json
echo '{}' > latest_scan_results.json
echo 'latest_scan_results.json 已清空'
```
...
with open('signal_tracker.csv') as f:
    ...
    header = next(reader)
    rows = list(reader)
print(f'signal_tracker.csv: header={header}, rows={len(rows)} (expect 0)')
with open('ai_memory.json') as f:
    mem = json.load(f)
print(f'ai_memory.json: keys={len(mem)} (expect 0)')
"
# Expected: rows=0, keys=0
```

- [ ] **Step 3: Commit**

```bash
git add signal_tracker.csv ai_memory.json latest_scan_results.json
git commit -m "chore: clear v6 data before v7 historical simulation"
```
...
**Files:**
- Create: `simulate_v7_from_may.py`

- [ ] **Step 1: 编写失败测试**

在 `simulate_v7_from_may.py` 开头先写测试片段：

```python
#!/usr/bin/env python3
"""v7 历史模拟回填 — 从 2026-05-01 逐日模拟选股+AI分析"""
...
def truncate_df_to_date(df, target_date):
    raise NotImplementedError("TDD: 先写测试")
...
```bash
python3 simulate_v7_from_may.py
# Expected: NotImplementedError: TDD: 先写测试
```
...
```python
def truncate_df_to_date(df, target_date):
    """截断 DataFrame 到 target_date（含当日，不含未来）。
...
```bash
python3 simulate_v7_from_may.py
# Expected: ✅ truncate_df_to_date 测试通过
```
...
git commit -m "feat: TDD truncate_df_to_date — no future data leak"
```
...
在 `simulate_v7_from_may.py` 末尾追加：

```python
# --- 测试: get_market_context_historical ---
def test_market_context_today_consistency():
    """验证: target_date=今天时，输出与 get_market_context() 一致"""
    ...
    live_ctx = screener.get_market_context()
    hist_ctx = get_market_context_historical(today)
    
    # 两个输出都不为空
    assert live_ctx and len(live_ctx) > 50, f"Live context too short: {live_ctx}"
    assert hist_ctx and len(hist_ctx) > 50, f"Historical context too short: {hist_ctx}"
    # 两者应该包含相同的指数名称
    for idx_name in ['上证', '深证', '创业板']:
        assert idx_name in hist_ctx, f"Missing {idx_name} in historical context"
    print("✅ get_market_context_historical 今日一致性测试通过")
```
...
```bash
python3 simulate_v7_from_may.py
# Expected: NameError: name 'get_market_context_historical' is not defined
```
...
def get_market_context_historical(target_date_str, index_data=None):
    """重建 target_date 当天的大盘快照，格式与 get_market_context() 一致。
...
```bash
python3 simulate_v7_from_may.py
# Expected: ✅ get_market_context_historical 今日一致性测试通过
```
...
```bash
git add simulate_v7_from_may.py
git commit -m "feat: TDD get_market_context_historical — historical market context"
```
...
在 `simulate_v7_from_may.py` 追加：

```python
# --- 测试: detect_regime_historical ---
...
```bash
python3 simulate_v7_from_may.py
# Expected: NameError: detect_regime_historical not defined
```
...
def detect_regime_historical(target_date_str, index_data=None):
    ...
    # 模式映射 (与 detect_market_regime 一致)
    if avg_trend < -0.5:
        regime = 'bear'
...
```bash
python3 simulate_v7_from_may.py
# Expected: ✅ detect_regime_historical 测试通过: X档: ... → strict/loose/bear
```
...
```bash
git add simulate_v7_from_may.py
git commit -m "feat: TDD detect_regime_historical — historical market regime"
```

---

### Task 4: 加载 v7 参数 + 数据预加载
...
```bash
python3 -c "
from simulate_v7_from_may import load_v7_params, preload_stock_cache
params = load_v7_params()
assert len(params) == 3
assert 'bear' in params and 'loose' in params and 'strict' in params
data = preload_stock_cache()
assert len(data) > 4000, f'Expected >4000 stocks, got {len(data)}'
print('✅ 参数加载 + 数据预加载 验证通过')
"
```
...
```bash
git add simulate_v7_from_may.py
git commit -m "feat: load v7 params + preload stock cache"
```
...
def simulate_day(target_date_str, all_data, v7_params, screener, index_data):
    ...
    target_dt = pd.Timestamp(target_date_str)
    cutoff_dt = target_dt - pd.Timedelta(days=30)  # 往前看30天
    ...
    for code in hot_codes:
        try:
            ...
            # 转换为 _screen_single_stock 期望的格式
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"✅ 模拟完成: {days_with_signals}/{len(trading_days)} 天有信号, 共 {total_candidates} 只候选")
    print(f"{'='*70}")
    
    # TODO: Task 6 (AI分析) + Task 7 (存储写入) + Task 8 (收益验证)
...
```bash
python3 simulate_v7_from_may.py 2>&1 | head -80
# 检查输出: 32个交易日, 预过滤数合理, 候选数可为0
```
...
```bash
git add simulate_v7_from_may.py
git commit -m "feat: simulate_day + main loop — v7 historical screening"
```
...
从 `auto_daily.py` 的 `_run_ai_analysis()` 复制核心逻辑，适配历史模拟环境：

```python
def run_ai_analysis_historical(code, stock_df, candidate, market_context, mode, scan_date_str, ai_memory):
    """对历史日期的候选股调用 DeepSeek API 分析。
    
    与 auto_daily._run_ai_analysis() 逻辑一致，但：
    - 使用传入的 scan_date_str 而非 datetime.now()
    - 使用传入的 market_context 而非 live get_market_context()
    - 接受已有的 ai_memory dict 避免重复读写文件
    
    Returns:
        dict: AI 分析记录 (写入 ai_memory 的格式), 或 None
    """
    import requests
    ...
    # 技术指标计算 (与 auto_daily._run_ai_analysis 完全一致)
...
## 基础指标
- 最新价：{current_price:.2f} | 均线：MA5={ma5:.2f} MA10={ma10:.2f} MA20={ma20_val:.2f}
...
- 最终结论：【参与 / 观望 / 放弃】"""
    ...
    try:
        ...
        # 导入 screener 的 DEEPSEEK_API_URL
        import importlib.util
        spec = importlib.util.spec_from_file_location("screener", os.path.join(BASE, "选股new_v5.py"))
...
```python
        ...
        for i, cand in enumerate(result['candidates']):
            ...
            record = run_ai_analysis_historical(
                ...
                result['mode'], day_str.replace('-', ''), ai_memory
            )
...
```bash
git add simulate_v7_from_may.py
git commit -m "feat: AI analysis integration for historical simulation"
```

---

### Task 7: 存储写入 (signal_tracker.csv + results_archive)
...
def save_signal_to_tracker(day_str, candidate, mode):
    ...
    new_row = [
        signal_date_str,           # date
        str(candidate.get('signal_date', '')),  # signal_date
        code,                       # code
        mode,                       # mode
        str(entry_price),          # entry_price
        str(candidate.get('pullback_pct', 0)),  # pullback_pct
        str(candidate.get('limit_days', 0)),    # limit_days
        '',                         # name (留空)
        '',                         # sector (留空)
    ]
...
def save_day_result_json(day_str, result):
    """保存当日结果到 results_archive/{YYYYMMDD}.json（格式与 auto_daily 兼容）。"""
...
```bash
git add simulate_v7_from_may.py
git commit -m "feat: storage — signal_tracker.csv + results_archive JSON"
```
    ...
    for code, records in ai_memory.items():
        for rec in records:
            if rec.get('verified'):
                continue
            try:
                rec_date = dt.strptime(rec['date'], '%Y%m%d')
...
```bash
git add simulate_v7_from_may.py
git commit -m "feat: post-hoc return verification for signals >7 days old"
```

---

### Task 9: 最终 main() 收尾 + 日志输出

- [ ] **Step 1: 更新 latest_scan_results.json**

```python
    # 更新 latest_scan_results.json（用最后一天的结果）
...
```bash
git add simulate_v7_from_may.py
git commit -m "feat: finalize main() — full v7 simulation pipeline"
```

---

### Task 10: 代码审查 (4-agent 并行审查)

使用 memory 中的审查工作流：

- [ ] **Step 1: 启动 4 个 Agent 并行扫描**

```bash
# Agent A: 审查 simulate_v7_from_may.py 筛选逻辑（截断+_screen_single_stock调用）
# Agent B: 审查 simulate_v7_from_may.py AI 分析部分（API调用+存储）
# Agent C: 审查 选股new_v5.py 是否有被历史模拟影响的副作用
# Agent D: 审查数据流完整性（signal_tracker.csv + ai_memory.json + results_archive/）
```

- [ ] **Step 2: 汇总去重**

- [ ] **Step 3: TDD 修复 CRITICAL + HIGH**
...
```bash
python3 -m py_compile simulate_v7_from_may.py
# Expected: 无输出（编译成功）
```
...
```bash
python3 -c "
from simulate_v7_from_may import (
    truncate_df_to_date, get_market_context_historical,
    detect_regime_historical, simulate_day, load_v7_params,
    preload_stock_cache, run_ai_analysis_historical,
)
print('✅ 所有函数导入成功')
"
```
...
```bash
# 只跑测试不跑完整流程
python3 simulate_v7_from_may.py --test
# Expected: 3个测试全部通过
```
...
# signal_tracker.csv 行数
with open('signal_tracker.csv') as f:
    rows = list(csv.reader(f))[1:]  # skip header
print(f'signal_tracker.csv: {len(rows)} 行')

# ai_memory.json 记录数
with open('ai_memory.json') as f:
    mem = json.load(f)
total_records = sum(len(v) for v in mem.values())
verified = sum(1 for v in mem.values() for r in v if r.get('verified'))
print(f'ai_memory.json: {total_records} 条记录 ({verified} 已验证)')

# results_archive 文件数
import os
archive = [f for f in os.listdir('results_archive') if f.endswith('.json')]
print(f'results_archive: {len(archive)} 个文件')
"
```

- [ ] **Step 5: Streamlit 复盘页展示验证**

```bash
streamlit run streamlit_app.py
# → 打开浏览器 → 复盘页面 → 确认有5月至今的数据
```
...
```bash
git add simulate_v7_from_may.py
git commit -m "feat: v7 historical simulation backfill — complete"
```
...
| # | 标准 | 验证方法 |
|---|------|---------|
| 1 | 无未来信息泄露 | `truncate_df_to_date` 单元测试通过 |
| 2 | 今日一致性 | target_date=今天 → 与 live get_market_context() 一致 |
| 3 | 存储完整性 | signal_tracker.csv 行数 + ai_memory.json 记录数 = candidates 数 |
| 4 | 复盘可展示 | Streamlit 复盘页能看到5月至今的信号+裁决 |
| 5 | AI 分析完整 | 每条记录有 sentiment/position/opinion |
| 6 | 收益可验证 | >7天前的信号有 return_7d/exit_reason/verdict |
| 7 | JSON 格式兼容 | results_archive JSON 结构匹配 auto_daily 输出 |

## 预计耗时

| 任务 | 预估 |
|------|------|
| Task 0-3: TDD 核心函数 | 15 min |
| Task 4-5: 参数加载+主循环 | 10 min |
| Task 6-7: AI分析+存储 | 15 min |
| Task 8-9: 收益验证+收尾 | 10 min |
| Task 10: 代码审查 | 20 min |
| Task 11: 最终验收 | 10 min |
| **开发总计** | **~80 min** |
| **运行 (32天×AI)** | **30-60 min** (取决于候选数量) |
