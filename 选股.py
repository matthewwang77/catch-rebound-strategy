...
def download_one_stock(code):
    ...
    try:
        ...
        df = ticker.history(start="2020-01-01", end="2026-05-25")
...
# ==================== 5. 识别连板 ====================
def identify_limit_up_series(df_stock):
    """识别连板序列"""
...
def run_backtest(start_date, end_date):
    ...
    for idx, fname in enumerate(cache_files):
        ...
        # 快速检查：有没有涨幅>=9.5%的（涨停）
        has_limit_up = (df['pct_chg'] >= 9.5).any()
    ...
    for idx, code in enumerate(hot_codes):
        ...
        # 向量化标注涨停和一字板
        df_stock['is_limit_up'] = df_stock['pct_chg'] >= 9.5
        ...
        for group in groups:
            ...
            # 检查回调：从连板结束后第 min_pullback_days 天开始
    ...
    # 保存
    output_file = f'backtest_signals_{start_date}_{end_date}.csv'
...
def screen_today(mode="normal"):
    ...
    for batch_idx, batch in enumerate(batches):
        print(f"  批次 {batch_idx+1}/{len(batches)}：下载 {len(batch)} 只...", end=" ")
        ...
        try:
            ...
            for code in batch:
                ...
                try:
                    ...
                    if len(latest_group) < PARAMS['min_consecutive_limit_up']:
                        continue
                    
                    stats['consecutive_ok'] += 1
                    
                    # 实体板比例
                    entity_count = sum(1 for i in latest_group if not df_recent.iloc[i]['is_one_word'])
                    entity_ratio = entity_count / len(latest_group) if len(latest_group) > 0 else 0
                    if entity_ratio < PARAMS['min_entity_board_ratio']:
                        continue
                    
                    stats['entity_ratio_ok'] += 1
                    
                    last_limit_idx = latest_group[-1]
                    ...
                    highest_price = max(df_recent.iloc[i]['high'] for i in latest_group)
                    ...
                    if PARAMS['volume_shrink_ratio'] < 10:  # 不是调试模式
                        limit_volumes = [df_recent.iloc[i]['volume'] for i in latest_group]
                        limit_avg_vol = np.mean(limit_volumes)
    ...
    if len(candidates) == 0:
        print("\n今日无符合条件的股票")
        print("\n建议:")
        print("  1. 使用宽松模式: python 选股.py --today loose")
        print("  2. 使用调试模式查看所有有连板的股票: python 选股.py --today debug")
        return []
    ...
    df.to_csv(f'candidates_{datetime.now().strftime("%Y%m%d")}_{mode}.csv', index=False)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ['--help', '-h']:
        ...
        print("")
        print("用法:")
        print("  python 选股.py --download        # 下载全量历史数据（首次运行）")
        print("  python 选股.py --today [模式]    # 当日选股")
        print("  python 选股.py --optimize        # 参数优化（基于历史数据）")
        print("  python 选股.py                   # 默认回测（2025-2026）")
        print("")
        print("当日选股模式:")
        ...
        print("")
        print("示例:")
        print("  python 选股.py --today normal")
        print("  python 选股.py --today loose")
        print("  python 选股.py --today debug")
        exit()
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--today':
        ...
        if len(candidates) > 0:
            ...
            print(f"\n建议尝试:")
            print(f"  python 选股.py --today loose   # 宽松模式")
            print(f"  python 选股.py --today debug   # 调试模式（查看所有连板股）")
        exit()
    ...
    print("默认模式：执行历史回测")
    print("=" * 60)
    print("提示：使用 python 选股.py --help 查看所有功能")
    print("")
...
# 查看帮助 python 选股.py --help

# 下载数据（首次运行）python 选股.py --download

# 正常模式选股（推荐）python 选股.py --today normal

# 宽松模式选股（市场不好时）python 选股.py --today loose

# 调试模式（看所有连板股）python 选股.py --today debug

# 严格模式（历史最优参数）python 选股.py --today strict

# 参数优化 python 选股.py --optimize
